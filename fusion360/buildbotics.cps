...
/**
  Sailormoon Specific Changes:
  - added delay property that inserts a between spindle start and program start
  - added pause property that inserts a M0 prompt after M3
*/ 
...
properties = {
  ...
  spindleDelay: {
    title: "Spindle Delay",
    description: "Sets the time in seconds to delay after the spindle speed has been set. Useful if you are facing problems with your spindle not reaching its speed quick enough before engaging with the material",
    group: "preferences",
    type: "integer",
    value: 0,
    scope: "post"
  ...
  spindlePause: {
    title: "Spindle Confirmation",
    description: "Adds a pause with a prompt (M0 Message) after the spindle start command has been issued (M3), that lets users confirm if the spindle is actually running",
    group: "preferences",
    type: "boolean",
    value: false,
    scope: "post"
...
function onSpindleSpeed(spindleSpeed) {
  ...
  if (getProperty("spindleDelay") > 0) {
    writeBlock(gFormat.format(4), pFormat.format(getProperty("spindleDelay")));
  }
  if (getProperty("spindlePause")) {
    //put in pause after spindle speed change
    writeBlock("M0 (MSG, Continue if the spindle is still running)");
  }
}
...
function startSpindle(tool, insertToolCall) {
  if (tool.type != TOOL_PROBE) {
    ...
    if (getProperty("spindlePause")) {
        writeBlock("M0 (MSG, Continue if the spindle is running)");
    }
  }
}
// <<<<< INCLUDED FROM include_files/startSpindle.cpi
